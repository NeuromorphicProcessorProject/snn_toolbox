...
model_list = os.listdir(os.path.join(tensorboard_path, 'relu_0.1_1_bias_regularizer3'))
...
model_init = load_model(os.path.join(
    tensorboard_path, 'relu_0.1_1_bias_regularizer3', model_list[-2]),
    {'clamped_relu_0.1_1.0': ClampedReLU(0.1, 1.0)}, False)
...
model.add(Conv2D(128, (3, 3), padding='same', activation=nonlinearity[0],
                 input_shape=(3, 32, 32), bias_regularizer=bias_regularizer[0]))
model.add(Conv2D(128, (3, 3), padding='same', activation=nonlinearity[1],
                 bias_regularizer=bias_regularizer[1]))
model.add(MaxPooling2D())

model.add(Conv2D(256, (3, 3), padding='same', activation=nonlinearity[2],
                 bias_regularizer=bias_regularizer[2]))
model.add(Conv2D(256, (3, 3), padding='same', activation=nonlinearity[3],
                 bias_regularizer=bias_regularizer[3]))
model.add(MaxPooling2D())

model.add(Conv2D(512, (3, 3), padding='same', activation=nonlinearity[4],
                 bias_regularizer=bias_regularizer[4]))
model.add(Conv2D(512, (3, 3), padding='same', activation=nonlinearity[5],
                 bias_regularizer=bias_regularizer[5]))
model.add(MaxPooling2D())

model.add(Flatten())

model.add(Dense(1024, activation=nonlinearity[6],
                bias_regularizer=bias_regularizer[6]))
model.add(Dense(1024, activation=nonlinearity[7],
